...
const RECITERS:Rec[]=[
  ...
  {id:"ghamdi",name:"سعد الغامدي",server:"https://server7.mp3quran.net/s_gmd",ev:"Sa_d_al-Ghaamidi_128kbps"}, // Highest bitrate
  ...
  const startHifz=useCallback(()=>{
    ...
    r.onresult=(e:any)=>{
      const normAr = (s:string) => {
        ...
        // Normalize hamzas
        t = t.replace(/[أإآء]/g, 'ا').replace(/ة/g, 'ه').replace(/ى/g, 'ي').replace(/ؤ/g, 'و');
        // Normalize phonetics (Kaf/Qaf, Sad/Seen, Dad/Zah, etc.)
        t = t.replace(/ق/g, 'ك')
             .replace(/ص/g, 'س')
             .replace(/ث/g, 'س')
             .replace(/ذ/g, 'ز')
             .replace(/ض/g, 'ظ');
        // Split into words, strip leading "ال" from each word
        return t.split(/\s+/).map(w => w.startsWith('ال') ? w.slice(2) : w).join(' ');
      };
