    ...
    <div className="select-none overflow-hidden" style={{background:colors.bg,color:colors.text,height:'100dvh'}}>
      ...
      <div className="overflow-y-auto"
        ...
        :<div className="flex flex-col justify-between px-4 md:px-6" style={{maxWidth:680,margin:'0 auto',width:'100%',minHeight:'100%',paddingTop:20,paddingBottom:8}}>
          ...
          {groups.map((g,gi)=>{const totalChars=groups.reduce((t,gg)=>t+gg.ayahs.reduce((s,a)=>s+a.text.length,0),0);const estLines=Math.ceil(totalChars/(window.innerWidth<500?18:24));const availH=window.innerHeight-80;const idealLH=availH/Math.max(estLines,1);const idealF=Math.min(Math.max(idealLH/1.75,18),36);const dynSize=idealF+'px';const dynLine=idealF>28?'1.85':idealF>24?'1.75':'1.7';return <div key={`${g.sn}-${gi}`}>
