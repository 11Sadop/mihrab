...
const pgNormalizeTextIndexed = (col: any) => {
    return sql`LEFT(
        translate(
...
async function test() {
    console.time("Total Indexed Search Time");
    ...
    try {
        ...
        if (keywords.length > 0) {
            const buildConditions = (textCol: any) => and(...keywords.map((kw: string) => ilike(pgNormalizeTextIndexed(textCol), `%${kw}%`)));
    ...
    console.timeEnd("Total Indexed Search Time");
}
