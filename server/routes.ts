...
export async function registerRoutes(
    ...
    app.get("/api/hadith/verify", async (req, res) => {
        try {
            ...
            const st = (req.query.st as string) || 'w';
            const t = (req.query.t as string) || '*';
            const xclude = req.query.xclude as string;
            const dedup = req.query.dedup as string;
            ...
            const executeDorarSearch = async (
                queryStr: string,
                options?: {
                    st?: string;
                    d?: string[];
                ...
                try {
                    ...
                    const stParam = options?.st || 'w';
                    const dParam = options?.d ? options.d.map(val => `&d[]=${val}`).join('') : '';
                    const tParam = options?.t ? `&t=${options.t}` : '';
                    const xParam = options?.xclude ? `&xclude=${encodeURIComponent(options.xclude)}` : '';

                    const apiUrl1 = `https://dorar.net/dorar_api.json?skey=${encodeURIComponent(queryStr)}&st=${stParam}${dParam}${tParam}${xParam}&page=1`;
                    const apiUrl2 = `https://dorar.net/dorar_api.json?skey=${encodeURIComponent(queryStr)}&st=${stParam}${dParam}${tParam}${xParam}&page=2`;
                    ...
                    if (list.length === 0 && !wasBlocked) {
                        try {
                            ...
                            const htmlUrl = `https://dorar.net/hadith/search?q=${encodeURIComponent(queryStr)}&st=${stParam}${dParam}${tParam}${xParam}`;
            ...
            if (gradeFilter === 'sahih' || gradeFilter === '1') {
                d = ['1'];
            } else if (gradeFilter === 'weak' || gradeFilter === '2') {
                d = ['2'];
            }
            ...
            const [localSearchResults, onlineResults] = await Promise.all([
                ...
                (async () => {
                    ...
                    try {
                        ...
                        const keywords = st === 'p' ? [queryNorm] : queryNorm.split(/\s+/)
                            .map((w: string) => normalizeAr(w))
                            .filter((w: string) => w.length >= 2 && !LOCAL_STOP_WORDS.has(w));

                        if (keywords.length > 0) {
                            const buildConditions = (textCol: any) => {
                                if (st === 'p') {
                                    return ilike(pgNormalizeText(textCol), `%${queryNorm}%`);
                                }
                                return and(...keywords.map((kw: string) => ilike(pgNormalizeText(textCol), `%${kw}%`)));
                            };

                            const isAcceptableOnly = d?.includes('1');
                            const isUnacceptableOnly = d?.includes('2');

                            const promises: Promise<any[]>[] = [];
                            
                            if (!isUnacceptableOnly) {
                                promises.push(db.select().from(bukhariHadiths).where(buildConditions(bukhariHadiths.text)).limit(15));
                                promises.push(db.select().from(muslimHadiths).where(buildConditions(muslimHadiths.text)).limit(15));
                            } else {
                                promises.push(Promise.resolve([]));
                                promises.push(Promise.resolve([]));
                            }

                            promises.push(db.select().from(verificationHadiths).where(buildConditions(verificationHadiths.text)).limit(25));

                            const [bkResults, muResults, vhResults] = await Promise.all(promises);
                            ...
                            for (const h of vhResults) {
                                const g = (h.status || '').toLowerCase();
                                const isSahih = g.includes("صحيح") || g.includes("حسن") || g.includes("جيد") || g.includes("ثابت") || g.includes("محفوظ");
                                const isWeak = g.includes("ضعيف") || g.includes("موضوع") || g.includes("باطل") || g.includes("منكر") || g.includes("لا يصح");
                                
                                if (isAcceptableOnly && !isSahih) continue;
                                if (isUnacceptableOnly && !isWeak) continue;
                ...
                (async () => {
                    let online: any[] = [];
                    try {
                        online = await executeDorarSearch(cleanedSearchKey, { st, d, t, xclude });
                        ...
                        if (online.length === 0 && queryWords.length > 4) {
                            ...
                            online = await executeDorarSearch(fallbackQuery, { st, d, t, xclude });
                        }
                    } catch (e: any) {
            ...
            results = results.filter((r: any) => {
                ...
                if (st === 'p') return false; // Exact match filter must strictly match phrase
            ...
            if (gradeFilter === 'sahih' || gradeFilter === '1') {
                results = results.filter((r: any) => {
                    const g = r.grade || '';
                    return g.includes('صحيح') || g.includes('حسن') || g.includes('جيد') || g.includes('ثابت') || g.includes('محفوظ');
                });
            } else if (gradeFilter === 'weak' || gradeFilter === '2') {
                results = results.filter((r: any) => {
                    const g = r.grade || '';
                    return g.includes('ضعيف') || g.includes('موضوع') || g.includes('باطل') || g.includes('منكر') || g.includes('لا يصح');
                });
            }
