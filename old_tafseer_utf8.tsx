...
const RECITERS:Rec[]=[
  {id:"maher",name:"┘à╪º┘ç╪▒ ╪º┘ä┘à╪╣┘è┘é┘ä┘è",server:"https://server12.mp3quran.net/maher",ev:"Maher_AlMuaiqly_128kbps"},
  ...
  {id:"ghamdi",name:"╪│╪╣╪» ╪º┘ä╪║╪º┘à╪»┘è",server:"https://server7.mp3quran.net/s_gmd",ev:"Sa_ood_ash-Shuraym_128kbps"},
...
const spaceMuqattaat=(t:string)=>{
  // Only add spacing for muqatta'at letters (╪º┘ä┘à, ╪¡┘à, etc.) - preserve all tashkeel
...
const TAFSEER_SOURCES=[
  {id:'ar.muyassar',name:'╪º┘ä╪¬┘ü╪│┘è╪▒ ╪º┘ä┘à┘è╪│╪▒',api:'alquran'},
  {id:'ar.jalalayn',name:'╪¬┘ü╪│┘è╪▒ ╪º┘ä╪¼┘ä╪º┘ä┘è┘å',api:'alquran'},
  {id:'1',name:'╪¬┘ü╪│┘è╪▒ ╪º╪¿┘å ┘â╪½┘è╪▒',api:'tafseer'},
  {id:'2',name:'╪¬┘ü╪│┘è╪▒ ╪º┘ä╪╖╪¿╪▒┘è',api:'tafseer'},
  {id:'3',name:'╪¬┘ü╪│┘è╪▒ ╪º┘ä┘é╪▒╪╖╪¿┘è',api:'tafseer'},
  {id:'4',name:'╪¬┘ü╪│┘è╪▒ ╪º┘ä╪│╪╣╪»┘è',api:'tafseer'},
  {id:'5',name:'╪¬┘ü╪│┘è╪▒ ╪º┘ä╪¿╪║┘ê┘è',api:'tafseer'},
  {id:'8',name:'╪¬┘ü╪│┘è╪▒ ╪º┘ä┘ê╪│┘è╪╖',api:'tafseer'},
];
...
export default function TafseerPage(){
  ...
  const startHifz=useCallback(()=>{
    ...
    if(!SR){alert("╪º┘ä┘à╪¬╪╡┘ü╪¡ ┘ä╪º ┘è╪»╪╣┘à ╪º┘ä╪¬╪╣╪▒┘ü ╪╣┘ä┘ë ╪º┘ä╪╡┘ê╪¬. ╪º╪│╪¬╪«╪»┘à Chrome");return;}
    const r=new SR();
    r.lang="ar-SA";
    r.continuous=true;
    ...
    r.onresult=(e:any)=>{
      ...
      for(let wi = 0; wi < ew.length; wi++) {
        ...
        if(bestMatch >= 0.7) {
          ...
          if(bestMatch < 0.95) {
            ...
            wrongWords.push(`"${target}" ΓåÉ ┘å╪╖┘é╪¬ "${bestWord}"`);
          }
        } else {
          wrongWords.push(`Γ¥î "${target}"`);
        }
      }
      ...
      if(completeRatio > 0.3 && completeRatio < 0.7) {
        setHifzFeedback({type:'ok', msg:`ΓÅ│ ${Math.round(completeRatio*100)}% ┘à┘å ╪º┘ä╪ó┘è╪⌐...`});
      }
      ...
      if(wrongWords.length > 0 && completeRatio >= 0.5 && completeRatio < 0.7) {
        setHifzFeedback({type:'wrong_pron', msg:'ΓÜá∩╕Å ╪¬╪¡┘é┘é ┘à┘å ╪º┘ä┘å╪╖┘é', details:wrongWords.slice(0,3)});
        setHifzStatus('pron');
      }
      
      // Γ£à Verse completed successfully
      if(completeRatio >= 0.70 && !isAdvancingRef.current) {
        isAdvancingRef.current = true;
        
        const hasErrors = wrongWords.length > 0;
        const isPerfect = completeRatio >= 0.95 && wrongWords.filter(w=>w.startsWith('Γ¥î')).length === 0;
        
        if(isPerfect) {
          setHifzFeedback({type:'ok', msg:'┘à┘à╪¬╪º╪▓! ╪ú╪¡╪│┘å╪¬ Γ£à'});
          setHifzStatus('ok');
        } else if(hasErrors && wrongWords.some(w=>w.startsWith('Γ¥î'))) {
          setHifzFeedback({type:'wrong_verse', msg:'ΓÜá∩╕Å ╪╡╪¡┘è╪¡ ┘à╪╣ ┘à┘ä╪º╪¡╪╕╪º╪¬', details:wrongWords.filter(w=>w.startsWith('Γ¥î')).slice(0,3)});
          setHifzStatus('pron');
        } else {
  ...
  return(
    <div className="select-none overflow-hidden" style={{background:colors.bg,color:colors.text,height:'100dvh'}}>
        ...
        <div className="w-full bg-card rounded-t-2xl shadow-2xl border-t border-border max-h-[55vh] overflow-y-auto" style={{paddingBottom:'env(safe-area-inset-bottom,8px)'}} onClick={e=>e.stopPropagation()}>
          ...
          <div className="p-3 space-y-3" dir="rtl">
            ...
            <button onClick={()=>{setShowOptions(false);setShowTafseer(true);setTafseerText('');setTafseerLoading(true);(() => {
              ...
              if(src?.api==='tafseer'){
                fetch(`https://api.quran-tafseer.com/tafseer/${src.id}/${selVerse.sn}/${selVerse.nis}`).then(r=>r.json()).then(d=>{setTafseerText(d.text||'┘ä╪º ┘è┘ê╪¼╪» ╪¬┘ü╪│┘è╪▒');setTafseerLoading(false);}).catch(()=>{setTafseerText('┘ü╪┤┘ä ┘ü┘è ╪¬╪¡┘à┘è┘ä ╪º┘ä╪¬┘ü╪│┘è╪▒');setTafseerLoading(false);});
              } else {
                fetch(`https://api.alquran.cloud/v1/ayah/${selVerse.sn}:${selVerse.nis}/${tafseerSource}`).then(r=>r.json()).then(d=>{setTafseerText(d.data?.text||'┘ä╪º ┘è┘ê╪¼╪» ╪¬┘ü╪│┘è╪▒');setTafseerLoading(false);}).catch(()=>{setTafseerText('┘ü╪┤┘ä ┘ü┘è ╪¬╪¡┘à┘è┘ä ╪º┘ä╪¬┘ü╪│┘è╪▒');setTafseerLoading(false);});
              }
            })();}} className="w-full py-2.5 rounded-xl bg-muted text-sm font-bold flex items-center justify-center gap-1"><BookOpen className="w-4 h-4 text-primary"/>╪º┘ä╪¬┘ü╪│┘è╪▒</button>
      ...
      {showTafseer&&selVerse&&<div className="fixed inset-0 z-[62] flex flex-col" style={{background:colors.bg}}>
        ...
        <div className="px-4 py-3 border-b" style={{borderColor:colors.border+'40'}} dir="rtl">
          <select value={tafseerSource} onChange={e=>{setTafseerSource(e.target.value);setTafseerLoading(true);setTafseerText('');            (() => {
              ...
              if(src?.api==='tafseer'){
                fetch(`https://api.quran-tafseer.com/tafseer/${src.id}/${selVerse.sn}/${selVerse.nis}`).then(r=>r.json()).then(d=>{setTafseerText(d.text||'┘ä╪º ┘è┘ê╪¼╪» ╪¬┘ü╪│┘è╪▒');setTafseerLoading(false);}).catch(()=>{setTafseerText('┘ü╪┤┘ä');setTafseerLoading(false);});
              } else {
                fetch(`https://api.alquran.cloud/v1/ayah/${selVerse.sn}:${selVerse.nis}/${e.target.value}`).then(r=>r.json()).then(d=>{setTafseerText(d.data?.text||'┘ä╪º ┘è┘ê╪¼╪» ╪¬┘ü╪│┘è╪▒');setTafseerLoading(false);}).catch(()=>{setTafseerText('┘ü╪┤┘ä');setTafseerLoading(false);});
              }
                    ...
                    {g.ayahs.map(a=>{
                      ...
                      return<span key={k} className="inline" data-v="1" id={`verse-${g.sn}-${a.nis}`}>
                        ...
                        <span className="inline-flex items-center justify-center align-middle" data-v="1"
                          ...
                          <span style={{position:'relative',zIndex:1,fontSize:'0.8em',fontFamily:'sans-serif',fontWeight:700,color:isP?'#22c55e':bookmarks.has(k)?'#ec4899':'#8b7355',lineHeight:1}}>{hidden?'╪ƒ':a.nis}</span>
      ...
      {playingSn>0&&<div className="fixed left-0 right-0 bottom-0 z-50 bg-card/90 backdrop-blur-md border-t border-border shadow-[0_-4px_25px_rgba(0,0,0,0.15)]" style={{paddingBottom:'env(safe-area-bottom,6px)'}}>
