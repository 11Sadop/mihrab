...
// Prevent infinite update loop by clearing the flag on startup
try {
...
if ('serviceWorker' in navigator && isProduction) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js', { updateViaCache: 'none' }).then((registration) => {
            ...
            // Removed the 5-minute interval check to prevent constant update prompts
            // Updates will be handled by the browser's default behavior (24h check)
            // or on next page visit if controlled.
        }).catch((error) => {
