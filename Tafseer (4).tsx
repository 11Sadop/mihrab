...
const RECITERS:Rec[]=[
  ...
  {id:"ghamdi",name:"سعد الغامدي",server:"https://server7.mp3quran.net/s_gmd",ev:"Saood_ash-Shuraym_128kbps"},
  ...
  {id:"hudhaify",name:"علي الحذيفي",server:"https://server10.mp3quran.net/hthfi",ev:"Hani_Rifai_192kbps"},
  {id:"banna",name:"محمود علي البنا",server:"https://server8.mp3quran.net/bna",ev:"mahmoud_ali_al_banna_32kbps"},
...
export default function TafseerPage(){
  ...
  const handleEnded = () => {
    ...
    if (!q) { setPlayingKey(""); setIsPlaying(false); return; }

    // If last verse in surah - try to continue to next surah automatically
    if (q.nis >= q.maxNis) {
      if (q.sn < 114) {
        const nextSn = q.sn + 1;
        const nextMaxNis = SURAHS.find(s=>s.id===nextSn)?.c || 1;
        playQueueRef.current = { sn: nextSn, nis: 1, maxNis: nextMaxNis };
        playVerse(nextSn, 1);
      } else {
        setPlayingKey(""); setIsPlaying(false); playQueueRef.current = null;
      }
      return;
    }
    ...
    // Seamless transition - swap to preloaded buffer immediately (no gap)
    if (nextPreloadedKey.current === nextKey) {
        ...
        if('mediaSession' in navigator){
          try {
            navigator.mediaSession.metadata = new MediaMetadata({
              title:`سورة ${SURAHS.find(s=>s.id===q.sn)?.n||''}`,
              artist:getReciter().name,
              album:`آية ${nextNis}`,
      ...
      // Not preloaded yet - load and play immediately
  ...
  const handleTimeUpdate = () => {
    ...
    // Preload next verse earlier (5s window for smoother handoff on slow networks)
    const q = playQueueRef.current;
    if (!q || !a.duration || a.duration <= 0) return;
    const remaining = a.duration - a.currentTime;

    if (remaining <= 5.0 && remaining > 0) {
      // Next target could be next verse, OR first verse of next surah
      let nextSn = q.sn, nextNis = q.nis + 1;
      if (nextNis > q.maxNis) {
        if (q.sn >= 114) return;
        nextSn = q.sn + 1;
        nextNis = 1;
      }
      const nextKey = `${nextSn}-${nextNis}`;
      if (nextPreloadedKey.current !== nextKey) {
        const rec = getReciter();
        const url = rec.ev ? `https://everyayah.com/data/${rec.ev}/${pad3(nextSn)}${pad3(nextNis)}.mp3` : `${rec.server}/${pad3(nextSn)}${pad3(nextNis)}.mp3`;
        const b = bufferAudio();
        if (b) {
          b.src = url;
          b.load();
  ...
  const ensureVerseVisible=(sn:number,nis:number)=>{
    ...
    if(data&&!data.some((a:any)=>a.sn===sn&&a.nis===nis)){
      // Verse not on current page - jump to its page
      fetch(`https://api.alquran.cloud/v1/ayah/${sn}:${nis}`).then(r=>r.json()).then(d=>{if(d?.data?.page)setPg(d.data.page);}).catch(()=>{});
    } else {
      // Smart scroll - only move if verse is out of comfortable viewport area
      setTimeout(()=>{
        const el=document.getElementById(`verse-${sn}-${nis}`);
        if(!el) return;
        const rect = el.getBoundingClientRect();
        const vh = window.innerHeight;
        // Comfort zone: 25% from top to 70% from top
        const topLimit = vh * 0.25;
        const bottomLimit = vh * 0.70;
        // Only scroll if verse is outside comfort zone (avoids jumpiness during recitation)
        if(rect.top < topLimit || rect.bottom > bottomLimit) {
          // Scroll to position verse ~35% down the viewport (reader-friendly)
  ...
  const skipNext=()=>{
    ...
    if(q.nis<q.maxNis){
      q.nis++;playVerse(q.sn,q.nis);
    } else if(q.sn<114){
      // Jump to next surah
      const nextSn=q.sn+1;
      const nextMax=SURAHS.find(s=>s.id===nextSn)?.c||1;
      playQueueRef.current={sn:nextSn,nis:1,maxNis:nextMax};
      playVerse(nextSn,1);
    }
  };
  const skipPrev=()=>{
    ...
    if(q.nis>1){
      ...
      // Jump to last verse of previous surah
      const prevSn=q.sn-1;
      const prevMax=SURAHS.find(s=>s.id===prevSn)?.c||1;
      playQueueRef.current={sn:prevSn,nis:prevMax,maxNis:prevMax};
      playVerse(prevSn,prevMax);
    }
  };
  ...
  const startHifz=useCallback(()=>{
    ...
    const r=new SR();
    r.lang="ar-SA";  // Saudi Arabic for better Quranic pronunciation
    r.continuous=true;
    r.interimResults=true;
    r.maxAlternatives=5; // Maximum alternatives for best accuracy
    ...
    // Levenshtein distance for proper edit distance
    ...
    // Accurate word similarity using normalized edit distance
    const wordSimilarity = (a:string, b:string):number => {
      if(a === b) return 1;
      if(!a || !b) return 0;
      // Short words - strict equality
      if(a.length <= 2 || b.length <= 2) return a === b ? 1 : 0;
      // Containment gives high score (handles verb forms)
      if(a.includes(b) || b.includes(a)) {
        const lenRatio = Math.min(a.length,b.length)/Math.max(a.length,b.length);
        return 0.85 + lenRatio*0.1;
      }
      // Use edit distance normalized by longer length
    ...
    // Reset on every start - scoped to THIS recognition session only
    let allTranscripts:string[] = [];
    let processedResultIndex = 0;
    let currentTargetIdx = hifzIdx; // Lock target when starting; update on advance
    
    r.onresult=(e:any)=>{
      if(!pq.data) return;
      const exp=pq.data[currentTargetIdx]; if(!exp) return;
      if(isAdvancingRef.current) return;

      // Build display text from latest result only (what user just said)
      let displayText = '';
      for(let i = 0; i < e.results.length; i++) {
        displayText += e.results[i][0].transcript + ' ';
      }

      const expNorm = normAr(exp.text);
      const expWords = expNorm.split(' ').filter((w:string) => w.length > 1);

      // Only process NEW results (not already seen) - prevents duplicate accumulation
      for(let i = processedResultIndex; i < e.results.length; i++) {
        const result = e.results[i];
        
        // Pick best alternative by comparing against expected verse words
        ...
        for(let j = 0; j < result.length; j++) {
          ...
          for(const w of words) {
            for(const ew of expWords){
              if(wordSimilarity(w, ew) > 0.7){ score++; break; }
            }
          }
          if(score > bestScore) { bestScore = score; bestText = result[j].transcript; }
        }
        
        // Only add to accumulated list if this result is final OR it's the latest interim
        if(result.isFinal) {
          const words = normAr(bestText).split(' ').filter((w:string) => w.length > 1);
          allTranscripts.push(...words);
          processedResultIndex = i + 1;
        }
      }

      // Add interim (latest non-final) words as a temporary layer for matching
      let interimWords:string[] = [];
      if(e.results.length > processedResultIndex) {
        const lastResult = e.results[e.results.length - 1];
        if(!lastResult.isFinal) {
          interimWords = normAr(lastResult[0].transcript).split(' ').filter((w:string) => w.length > 1);
        }
      }

      hifzTxtRef.current = displayText;
      setRecTxt(displayText.split(' ').slice(-6).join(' '));

      // Combine committed + interim for matching (interim doesn't accumulate)
      const spokenWords = [...allTranscripts, ...interimWords];
      // Use sliding window - last N words where N ≈ 2x expected length
      const windowSize = Math.max(expWords.length * 2 + 4, 15);
      const recentSpoken = spokenWords.slice(-windowSize);
      
      // Sequential word-by-word matching with pointer to avoid double-counting
      let matchedCount = 0;
      let wrongWords:string[] = [];
      let spokenPtr = 0; // Position in recentSpoken we've consumed

      for(let wi = 0; wi < expWords.length; wi++) {
        const target = expWords[wi];
        let bestMatch = 0;
        let bestWord = '';
        let bestPtr = -1;

        // Search forward from current pointer (allow small lookback for out-of-order)
        const searchStart = Math.max(0, spokenPtr - 2);
        const searchEnd = Math.min(recentSpoken.length, spokenPtr + 6);
        for(let si = searchStart; si < searchEnd; si++) {
          const said = recentSpoken[si];
          const sim = wordSimilarity(said, target);
          if(sim > bestMatch) { bestMatch = sim; bestWord = said; bestPtr = si; }
        }
        ...
        if(bestMatch >= 0.75) {
          ...
          spokenPtr = bestPtr + 1;
          if(bestMatch < 0.92) {
            wrongWords.push(`"${target}" ← "${bestWord}"`);
          }
        } else if(bestMatch >= 0.5) {
          // Partial match - likely pronunciation issue
          wrongWords.push(`⚠️ "${target}" ≠ "${bestWord}"`);
        }
      }
      
      const completeRatio = expWords.length > 0 ? matchedCount / expWords.length : 0;
      
      // Real-time progress feedback
      if(completeRatio > 0.25 && completeRatio < 0.70) {
        setHifzFeedback({type:'ok', msg:`⏳ ${Math.round(completeRatio*100)}% من الآية...`});
      }
      
      // Pronunciation warnings during progress
      if(wrongWords.length > 0 && completeRatio >= 0.4 && completeRatio < 0.70) {
        setHifzFeedback({type:'wrong_pron', msg:'⚠️ تحقق من النطق', details:wrongWords.slice(0,3)});
        setHifzStatus('pron');
      }
      
      // ✅ Verse completed successfully - raised to 70% for stricter accuracy
      if(completeRatio >= 0.70 && !isAdvancingRef.current) {
        isAdvancingRef.current = true;
        
        const seriousErrors = wrongWords.filter(w=>w.startsWith('⚠️'));
        const minorErrors = wrongWords.filter(w=>!w.startsWith('⚠️'));
        const isPerfect = completeRatio >= 0.90 && seriousErrors.length === 0 && minorErrors.length <= 1;
        
        if(isPerfect) {
          setHifzFeedback({type:'ok', msg:'ممتاز! أحسنت ✅'});
          setHifzStatus('ok');
        } else if(seriousErrors.length > 0) {
          setHifzFeedback({type:'wrong_verse', msg:'⚠️ صحيح مع ملاحظات', details:seriousErrors.slice(0,3)});
          setHifzStatus('pron');
        } else {
        ...
        setTimeout(()=>{
          // Fully reset session state for next verse
          allTranscripts = [];
          processedResultIndex = 0;
          
          setHifzIdx(prev=>{
            const next=Math.min(prev+1,(pq.data?.length||1)-1);
            currentTargetIdx = next; // Update closure target
            setTimeout(()=>{
          ...
          // Restart recognition cleanly for new verse (shorter gap)
          try { r.abort(); } catch {}
        ...
        :<div className="flex-1 w-full max-w-2xl mx-auto px-4 md:px-8 relative min-h-full">
            <div className={`flex flex-col pt-32 pb-32 ${groups.reduce((t,gg)=>t+gg.ayahs.length,0)<15?'justify-center min-h-[70vh]':''}`}>
              {groups.map((g,gi)=>{
                ...
                const dynSize=allChars<300?'clamp(28px, 7.5vw, 42px)':
                               allChars<500?'clamp(25px, 7vw, 37px)':
                               allChars<800?'clamp(22px, 6vw, 33px)':
                               allChars<1200?'clamp(20px, 5.3vw, 30px)':
                               'clamp(18px, 4.8vw, 26px)';
                const dynLine=allChars<300?'3.4':allChars<500?'3.0':allChars<800?'2.7':allChars<1200?'2.5':'2.3';
                ...
                return <div key={`${g.sn}-${gi}`} className="relative w-full">
                    ...
                    <span onClick={()=>{if(!hifz){setSelVerse({sn:g.sn,nis:1,text:g.ayahs[0].orig});setShowOptions(true);}}}
                      ...
                      {g.sn===1 && (
                        <span className="inline-flex items-center justify-center align-middle"
                          style={{width:'2.2em',height:'2.2em',fontSize:'0.55em',verticalAlign:'middle',position:'relative',margin:'0 0.45em'}}>
                          <svg viewBox="0 0 50 50" width="100%" height="100%" style={{position:'absolute',inset:0,filter:(playingKey===`${g.sn}-1`)?'drop-shadow(0 2px 6px rgba(34,197,94,0.4))':'none'}}>
                            <circle cx="25" cy="25" r="22" fill={(playingKey===`${g.sn}-1`)?'rgba(34,197,94,0.18)':'rgba(200,169,110,0.1)'} stroke={(playingKey===`${g.sn}-1`)?'#22c55e':'#c8a96e'} strokeWidth="1.5"/>
                            <circle cx="25" cy="25" r="18" fill="none" stroke={(playingKey===`${g.sn}-1`)?'#22c55e':'#c8a96e'} strokeWidth="0.4" opacity="0.4"/>
                          </svg>
                          <span style={{position:'relative',zIndex:1,fontSize:'0.95em',fontFamily:'"Scheherazade New","Amiri",serif',fontWeight:700,color:(playingKey===`${g.sn}-1`)?'#22c55e':'#8b7355',lineHeight:1}}>١</span>
                        </span>
                      )}
                    </span>
                  </div>}
                  ...
                  {/* Ayahs Grid */}
                  <div className="text-justify font-quran" dir="rtl" style={{fontSize:dynSize,lineHeight:dynLine,fontWeight:'normal',letterSpacing:'0.01em',color:colors.text, wordSpacing:'0.12em', textAlignLast:'center', direction:'rtl', textAlign:'justify'}}>
                    {g.ayahs.map(a=>{
                      ...
                      return<span key={k} className="inline" data-v="1" id={`verse-${g.sn}-${a.nis}`}>
                        <span onClick={e=>{e.stopPropagation();if(!hifz){setSelVerse({sn:g.sn,nis:a.nis,text:a.orig});setShowOptions(true);}}}
                          ...
                          style={{
                            ...
                            padding:(isP||isSel)?'6px 12px':'0',borderRadius:(isP||isSel)?'12px':'0',
                            boxShadow:isP?'0 2px 12px rgba(34,197,94,0.25)':'none',
                          }}>
                          {a.text}
                        </span>
                        <span className="inline-flex items-center justify-center align-middle" data-v="1"
                          style={{width:'2.2em',height:'2.2em',fontSize:'0.55em',verticalAlign:'middle',position:'relative',display:'inline-flex',margin:'0 0.45em'}}>
                          <svg viewBox="0 0 50 50" width="100%" height="100%" style={{position:'absolute',inset:0,filter:isP?'drop-shadow(0 2px 6px rgba(34,197,94,0.4))':'none',transition:'filter 0.3s'}}>
                            <circle cx="25" cy="25" r="22" fill={isP?'rgba(34,197,94,0.18)':bookmarks.has(k)?'rgba(236,72,153,0.1)':'rgba(200,169,110,0.1)'} stroke={isP?'#22c55e':bookmarks.has(k)?'#ec4899':'#c8a96e'} strokeWidth="1.5"/>
                            <circle cx="25" cy="25" r="18" fill="none" stroke={isP?'#22c55e':bookmarks.has(k)?'#ec4899':'#c8a96e'} strokeWidth="0.4" opacity="0.4"/>
                          </svg>
                          <span style={{position:'relative',zIndex:1,fontSize:'0.95em',fontFamily:'"Scheherazade New","Amiri",serif',fontWeight:700,color:isP?'#22c55e':bookmarks.has(k)?'#ec4899':'#8b7355',lineHeight:1}}>{hidden?'؟':a.nis.toLocaleString('ar-EG')}</span>
            ...
            <div className="mt-12 mb-20 flex flex-col items-center gap-4">
                ...
                <div className="hidden md:flex items-center gap-3">
                    <button onClick={()=>{if(pg<604){setPg(p=>p+1);resetHifz();window.scrollTo(0,0);}}}
                        disabled={pg>=604}
                        className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all hover:scale-105 disabled:opacity-30"
                        style={{background:colors.border+'25',color:colors.text,border:'1px solid '+colors.border+'40'}}>
                        <ChevronRight className="w-4 h-4"/>
                        الصفحة التالية
                    </button>
                    <span className="text-xs opacity-40 px-3" style={{color:colors.text}}>{pg} / 604</span>
